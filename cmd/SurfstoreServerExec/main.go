...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType == "meta" {
		//fmt.Printlnln("Registering MetaStore Service")
		pb.RegisterMetaStoreServer(grpcServer, pb.NewMetaStore(blockStoreAddrs))
	}

	if serviceType == "block" {
		pb.RegisterBlockStoreServer(grpcServer, pb.NewBlockStore())
	}
